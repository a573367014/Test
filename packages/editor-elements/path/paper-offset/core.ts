        ...
        if (Math.abs(midOffset - Math.abs(offset)) > threshold) {
            ...
            if (subCurve != null) {
                return [
                    ...
                    ...adaptiveOffsetCurve(subCurve, offset),
                ];
            }
        }
    }
...
function normalize(path: PathType, areaThreshold = 0.01) {
    if (path.closed) {
        ...
        if (path instanceof paper.CompoundPath) {
            path.children
                .filter((c) => Math.abs((c as PathType).area) < ignoreArea)
                .forEach((c) => c.remove());
            if (path.children.length === 1) {
...
function isSameDirection(partialPath: paper.Path, fullPath: PathType) {
    ...
    const offset2 =
        partialPath.segments[Math.max(1, Math.floor(partialPath.segments.length / 2))].location
            .offset;
...
function offsetSimpleShape(
    ...
    const newPath = removeIntersection(
        new paper.Path({ segments, insert: false, closed: path.closed }),
    );
    newPath.reduce({});
