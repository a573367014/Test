...
function getComments(cnode, trailing) {
    ...
    const commentNodes = trailing ? cnode.trailingComments || [] : cnode.leadingComments || [];
    ...
    commentNodes.forEach((node) => {
        if (isCommentLine(node)) {
            if (isCodeBlockDeclaration(node.value) && codeBlockStarted) codeBlockStarted = false;
            comments = codeBlockStarted ? node.value.replace(/^\s/, '') : node.value.trim();
            if (isCodeBlockDeclaration(node.value) && typeof codeBlockStarted === 'undefined')
...
function filterBlockComments(comments) {
    ...
    return comments
        .map((t) => {
            if (isCodeBlockDeclaration(t) && codeBlockStarted) codeBlockStarted = false;
...
function isVueOption(path$$1, optionsName, componentLevel) {
    if (
        isValidObjectProperty(path$$1.node) &&
        path$$1.parentPath &&
        ...
        path$$1.parentPath.parentPath.node.callee.name === 'Component' &&
...
function getTypeByDataNode(node) {
    ...
    if (bt.isLiteral(dataNode) || (bt.isExpression(dataNode) && !bt.isBinaryExpression(dataNode))) {
...
function getEmitDecorator(decorators) {
    ...
    for (let i = 0; i < decorators.length; i++) {
        ...
        if (bt.isCallExpression(exp) && bt.isIdentifier(exp.callee) && exp.callee.name === 'Emit') {
            return decorators[i];
        }
    }
    return null;
...
function determineChildren(functionPath, onSlot) {
    ...
    contextBinding.referencePaths.forEach((refPath) => {
        if (
            ...
            slotRes.backerDesc = commentsRes.content ? commentsRes.content.join('') : '';
...
function parseJavascript(ast, seenEvent, options, source = '') {
    ...
    const vueComponentVisitor = {
        ...
        ObjectProperty(path$$1) {
            const { onProp, onMethod, onComputed, onName, onSlot, onMixIn, onData, onWatch } =
                options;
            // Processing name
            ...
            if (
                ...
                const properties = path$$1.node.value.properties.filter(
            ...
            if (onData && isVueOption(path$$1, 'data', componentLevel)) {
                path$$1.node.body.body.forEach((body) => {
                    if (bt.isReturnStatement(body)) {
                        const properties = body.argument.properties.filter(
                            (n) => bt.isObjectMethod(n) || bt.isObjectProperty(n),
                        );
                        properties.forEach((node) => {
                            const commentsRes = getComments(node);
        ...
        CallExpression(path$$1) {
            ...
            if (
                ...
                const parentExpressionStatementNode = path$$1.findParent((path$$1) =>
                    bt.isExpressionStatement(path$$1),
                );
                ...
                const scopedSlots = {
                    ...
                    backerDesc: slotsComments.content ? slotsComments.content.join('') : '',
                    bindings: {},
                    scoped: true,
                    target: 'script',
                };
        ...
        ClassProperty(path$$1) {
            ...
            if (propDecorator) {
                ...
                const { propName, options: propDecoratorArg } =
        ...
        MemberExpression(path$$1) {
            ...
            if (
                ...
                if (bt.isMemberExpression(parentNode) && bt.isIdentifier(parentNode.property)) {
                    // (this || vm).$slots.xxx
...
function getExportDefaultReferencePath(programPath) {
    ...
    Object.keys(bindings).forEach((key) => {
        bindings[key].referencePaths.forEach((path$$1) => {
            if (
                bt.isExportDefaultDeclaration(path$$1.parent) ||
...
function parseTemplate$$1(templateAst, seenEvent, options) {
    ...
    if (templateAst.attrsMap) {
        for (const [attr, value] of Object.entries(templateAst.attrsMap)) {
            if ((attr.startsWith('v-on:') || attr.startsWith('@')) && /\$emit\(.*?\)/.test(value)) {
                try {
        ...
        if (templateAst.tag === 'slot') {
            ...
            if (parent) {
                ...
                for (let i = 0; i < copies.length; i++) {
                    ...
                    if (el.isComment && !(parent.tag === 'slot' && parent.children[0] === el)) {
    ...
    traverse__default(astFile, {
        CallExpression(path$$1) {
            ...
            if (bt.isIdentifier(node.callee) && node.callee.name === '$emit') {
                const parentExpressionStatementNodePath = path$$1.findParent((path$$1) =>
                    bt.isExpressionStatement(path$$1),
                );
