...
const initShader = (gl, defines, vs, fs) => {
    const defineStr = Object.keys(defines).reduce(
        (str, key) => (defines[key] ? str + `#define ${key} ${defines[key]}\n` : ''),
        '',
    );
...
export const draw = (gl, plugin, dataBuffers, indexResource, uniforms, textures) => {
    const { schema, shaderRefs } = plugin;
    gl.useProgram(shaderRefs.program);
    Object.keys(shaderRefs.attributes).forEach((key) => {
        if (!schema.buffers[key] || schema.buffers[key].type === SchemaTypes.index) return;
