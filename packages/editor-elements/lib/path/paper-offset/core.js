...
function offsetSegment(segment, curve, handleNormal, offset) {
  var isFirst = segment.curve === curve; // get offset vector

  var offsetVector = curve.getNormalAtTime(isFirst ? 0 : 1).multiply(offset); // get offset point

  var point = segment.point.add(offsetVector);
  var newSegment = new paper.Segment(point); // handleOut for start segment & handleIn for terminal segment
...
function adaptiveOffsetCurve(curve, offset) {
  ...
  var segment2 = offsetSegment(curve.segment2, curve, hNormal, offset); // divide && re-offset

  var offsetCurve = new paper.Curve(segment1, segment2); // if the offset curve is not self intersected, divide it
  ...
  if (offsetCurve.getIntersections(offsetCurve).length === 0) {
    ...
    if (Math.abs(midOffset - Math.abs(offset)) > threshold) {
      ...
      if (subCurve != null) {
        return [].concat(adaptiveOffsetCurve(curve, offset), adaptiveOffsetCurve(subCurve, offset));
      }
    }
  }
  ...
  if (origin.isSmooth()) {
    ...
    if (intersection.length === 0) {
      if (distance > Math.abs(offset) * 0.1) {
        ...
        switch (joinType) {
          case 'miter':
            var join = getPointLineIntersections(curve1.point2, curve1.point2.add(curve1.getTangentAtTime(1)), curve2.point1, curve2.point1.add(curve2.getTangentAtTime(0))); // prevent sharp angle
      ...
      if (second1) {
        var _join = second1.segment1;
        var second2 = curve2.divideAt(curve2.getIntersections(curve1)[0]);
        _join.handleOut = second2 ? second2.segment1.handleOut : segments2[0].handleOut;
        segments1.pop();
        segments2[0] = _join;
      } else {
        segments2[0].handleIn = segments1[1].handleIn;
...
function normalize(path, areaThreshold) {
  ...
  if (path.closed) {
    var ignoreArea = Math.abs(path.area * areaThreshold);
    ...
    if (path instanceof paper.CompoundPath) {
      path.children.filter(function (c) {
        return Math.abs(c.area) < ignoreArea;
      }).forEach(function (c) {
        return c.remove();
      });
...
function offsetSimpleShape(path, offset, join, limit) {
  ...
  var _preparePath = preparePath(path, offset);

  source = _preparePath[0];
  offset = _preparePath[1];
  var curves = source.curves.slice();
  if (!curves.length) return path;
  ...
  var newPath = removeIntersection(new paper.Path({
    segments: segments,
    insert: false,
    closed: path.closed
  }));
  ...
  switch (cap) {
    case 'round':
      ...
      var result = new paper.Path({
        segments: [].concat(heads, oSegments, tails, iSegments),
        closed: true,
        insert: false
      });
      result.reduce({});
    ...
    default:
      return new paper.Path({
        segments: [].concat(oSegments, iSegments),
        closed: true,
        insert: false
      });
  }
...
function offsetSimpleStroke(path, offset, join, cap, limit) {
  ...
  if (path.closed) {
    ...
    if (holes.length > 0) {
      for (var _iterator = _createForOfIteratorHelperLoose(holes), _step; !(_step = _iterator()).done;) {
        var hole = _step.value;
        final = final.subtract(hole, {
          insert: false
        });
      }
    }
...
export function offsetPath(path, offset, join, limit) {
...
export function offsetStroke(path, offset, join, cap, limit) {
  var nonSIPath = getNonSelfItersectionPath(path);
  var result = nonSIPath;
