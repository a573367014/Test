...
exports.getExpandedEntryPointsForPaths =
  exports.getWatchEntryPoints =
...
function getWatchEntryPoints(logger, options, program) {
  ...
  switch (options.getValue("entryPointStrategy")) {
    case exports.EntryPointStrategy.Resolve:
      result = getEntryPointsForPaths(logger, entryPoints, options, [program]);
      break;
    case exports.EntryPointStrategy.Expand:
      result = getExpandedEntryPointsForPaths(logger, entryPoints, options, [
        program,
...
function getModuleName(fileName, baseDir) {
  return (0, fs_1.normalizePath)(
    (0, path_1.relative)(baseDir, fileName)
  ...
  entryLoop: for (const fileOrDir of inputFiles.map(fs_1.normalizePath)) {
    ...
    for (const program of programs) {
      for (const check of toCheck) {
        ...
        const sourceFile = program.getSourceFile(check);
        // console.log("sourceFile", sourceFile, check);
        if (sourceFile) {
          entryPoints.push({
...
function getExpandedEntryPointsForPaths(
  logger,
  inputFiles,
  options,
  programs = getEntryPrograms(logger, options)
...
function getEntryPrograms(logger, options) {
  ...
  if (rootProgram.getRootFileNames().length === 0) {
    ...
    for (const ref of resolvedReferences !== null &&
    resolvedReferences !== void 0
      ? resolvedReferences
      : []) {
...
function getEntryPointsForPackages(logger, packageGlobPaths, options) {
  ...
  const expandedPackages = (0, package_manifest_1.expandPackages)(
    logger,
    ".",
    packageGlobPaths
  );
  for (const packagePath of expandedPackages) {
    ...
    const packageJson = (0, package_manifest_1.loadPackageManifest)(
      logger,
      packageJsonPath
    );
    ...
    const packageEntryPoint = (0, package_manifest_1.getTsEntryPointForPackage)(
      logger,
      packageJsonPath,
      packageJson
    );
