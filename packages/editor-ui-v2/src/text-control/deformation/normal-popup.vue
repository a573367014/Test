...
import { createDeformations } from './normal';
import pick from 'lodash/pick';
import { i18n } from '../../i18n';

let deformations = null;
...
export default {
    ...
    data() {
        deformations = createDeformations();
    ...
    computed: {
        currentEffect() {
            ...
            if (deformation && deformation.enable) {
        ...
        deformation() {
            ...
            if (this.element.deformation && this.element.deformation.enable) {
            ...
            for (let index = 0; index < deformations.length; ++index) {
                ...
                if (el.type === type) {
                    ...
                    this.setIndexShowPanel(row * 3 - 1);
                    break;
                }
            }

            if (this.indexShowPanel >= deformations.length) {
                this.setIndexShowPanel(deformations.length - 1);
            }
    ...
    methods: {
        ...
        effectClick(item) {
            if (item.id === 'none') {
                this.editor.convertEffectTextToText(this.element);
                this.$emit('close');
                return;
            }
        ...
        emitChange(element) {
            this.editor.$emit('change', element || this.element);
            setTimeout(() => {
                this.editor.makeSnapshotByElement(element || this.element, false, true);
            }, 500);
        },
        ...
        getValue(index) {
            const { deformation } = this.element;
            if (deformation && deformation.enable) {
                return deformation.intensitys[index];
            } else {
                return 0;
            }
        },
        ...
        handelTransformSelect(currentEffect) {
            ...
            let { deformation } = this.element;
            if (currentElement.type !== 'effectText') {
                const data = {
                    type: 'effectText',
                    deformation: {
                        enable: true,
                    ...
                    animations: this.element.animations, // 同步动画相关
                };
                Object.assign(data, extractTextCore(currentElement));
                currentElement = this.editor.replaceElement(currentElement, data);
                this.editor.focusElement(currentElement);
                deformation = currentElement.deformation;
            }
            ...
            if (deformation.type !== currentEffect.id || !deformation.enable) {
                ...
                if (currentEffect.isFollowTangent) {
                    deformation.isFollowTangent = currentEffect.isFollowTangent;
                }
            }

            this.emitChange(currentElement);
        },

        handleDeformationClear() {
            const currentElement = this.element;
            currentElement.deformation.enable = false;
            this.emitChange();
        },

        handleIntensityChange(val, index, attr) {
            if (attr.name === i18n.$tsl('边数')) {
                val = Math.round(val);
            }
            this.element.$needSpeedUp = true;
